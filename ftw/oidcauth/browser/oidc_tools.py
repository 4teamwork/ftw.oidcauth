...
class OIDCClientAuthentication(object):
    ...
    def authorize(self):
        ...
        user_id = self._oidc_plugin._username_attribute
        # TODO: make properties mapping configurable in ZMI
        properties = {'fullname': user_info.get('name'),
                      'email': user_info.get('email')}
        oidc_user_handler = OIDCUserHandler(self._request, user_id, properties)
...
class OIDCUserHandler(object):
    def __init__(self, request, userid, properties):
